...
% Implementation of the remez procedure (instrucitons on e-classroom)
...
function [P, m, endcondition, E1] = RemezStep(f, a, b, n, tol, E0)
    ...
    y = xx(idx);
    if abs(Mn - abs(m)) < tol
        endcondition = 1; return
    else
        endcondition = 0;
        %change of points in set E0
        idx1 = -1;
        for j = 1:n+1
            if (E0(j) - y) <= 0 && (E0(j+1) - y > 0)
                idx1 = j;
            end
        end
        if y >= E0(end) 
        ...
        else % y is somewhere on the [x_{idx1}, x_{idx1+1}]
            if sign((-1)^(idx1+1) *m) == sign(f(y - polyval(P,y)))
                E1(idx1) = y;
            else 
                E1(idx1 +1) = y;
            end
